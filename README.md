...
| `cooldown` | 同一用户的冷却时间（秒） | `5.0` |
| `group_cooldown` | 同一群任意用户之间的全局冷却（秒，`0` 为关闭） | `15.0` |
| `blacklisted_users` | 用户黑名单 | `[]` |
| `poke_prompt` | 提示词模板，支持 `$username`、`$user_id`、`$scene` | 见下方 |

### 默认提示词

默认提示词会先定位触发者本人最近未得到回应的内容；如果只是随手戳，则要求 LLM 结合现有人设、双方关系和群聊气氛给出一两句即时反应，避免误接其他群友的话、固定撒娇或机械重复“怎么了”。

`$username`、`$user_id` 和 `$scene` 会分别替换为触发者昵称、用户 ID 与群聊/私聊场景。旧版只使用 `$username` 的自定义提示词仍然兼容。
...
戳一戳属于通知事件，没有可供 OneBot 引用的真实消息 ID。插件会保留其他发送前处理，但仅对戳一戳触发的回复跳过引用消息段；普通消息的引用回复配置不受影响。

### 内存管理

- 群聊同时执行个人冷却和群级冷却，避免多人轮流戳绕过限制
- AstrBot 全局管理员不受任何冷却限制，也不会启动普通成员的冷却
- 私聊只执行个人冷却，不受群级冷却影响
- 只有实际触发回复的戳会启动冷却，被拦截的戳不会延长冷却
- 冷却记录至少保留 10 分钟；配置更长冷却时会相应延长
- 每 50 次戳一戳触发一次清理
- 无内存泄漏风险
...
- 个人与群级双层冷却防止单人刷屏或多人轮流触发
- AstrBot 全局 `admins_id` 中的管理员始终可以触发，不受冷却影响
- 对话会正常记入对话历史
